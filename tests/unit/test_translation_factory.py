...
from translator_app.core.errors import (
    TranslationTypeNotFoundError,
...
def test_invalid_dialect_for_eng_to_pig_latin_translator():
    with pytest.raises(UnsupportedDialectError):
        translator_type = TranslatorTypeEnum("eng_to_pig_latin")
        get_translator(translator_type, "randim dialect name")


def test_invalid_type_for_eng_to_pig_latin_translator():
    with pytest.raises(TranslationTypeNotFoundError):
...
async def test_valid_eng_to_pig_latin_dialect_ay(input_text, expected_text):
    translator_type = TranslatorTypeEnum("eng_to_pig_latin")
    translator = get_translator(translator_type, PigLatinDialectEnum.AY.value)
...
async def test_valid_eng_to_pig_latin_dialect_way(input_text, expected_text):
    translator_type = TranslatorTypeEnum("eng_to_pig_latin")
...
async def test_valid_eng_to_pig_latin_dialect_yay(input_text, expected_text):
    translator_type = TranslatorTypeEnum("eng_to_pig_latin")
